...
def format_gain_output(df, group_cols, preferred_cols, exclude_cols):
    ...
    leading.extend([c for c in ["Ft_mean", "Fmax_mean"] if c in df.columns])

    final_cols = build_final_columns(df, leading, available, exclude_cols)
